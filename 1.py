...
Основные функции:
    • нажатием F12 открывается консольное меню;
    • можно добавить «разовую» команду или BUMP‑задачу с автопарсингом;
    • парсер умеет распознавать любые варианты записи времени
      (русские формы, сокращения, английские единицы, любой порядок);
    • после получения сообщения «/remaining» бот вытаскивает оставшееся время,
      планирует /up, /bump и /like и исполняет их в нужный момент;
    • расписание сохраняется в `schedule.json`, пользовательские ответы –
      в `responses.json`;
    • весь вывод пишется в консоль, при желании также в `bot.log`.

Требуемые пакеты (pip):
...
# хранилище отложенных (однократных) команд
scheduled_tasks: List[Dict[str, Any]] = []      # будет сохраняться в schedule.json

# BUMP‑задачи (автопарсинг)
bump_tasks: List[Dict[str, Any]] = []         # сохраняются только в текущей сессии

# ----------------------------------------------------------------------
# LOGGING HELPERS
# ----------------------------------------------------------------------
def _log(msg: str, level: str = "INFO") -> None:
    """Выводит сообщение в консоль и (при необходимости) в файл."""
...
def format_seconds(seconds: int) -> str:
    """`Xч Yм Zс` из количества секунд (для красивого вывода)."""
    if seconds < 0:
...
def parse_duration_to_seconds(text: str) -> Optional[int]:
    ...
    Поддерживает:
        • русские формы (часов, минут, секунд, падежи);
        • сокращения («2ч44м35с», «2 ч 44 м 35 с»);
        • английские аналоги (h, m, s);
        • любой порядок единиц;
        • набор из 3‑х цифр без указаний единиц → трактуем как ЧЧ:ММ:СС.
    """
    try:
        s = text.lower()
        # 1) убираем знаки пунктуации и «в, на, и, до» и т.п.
        s = re.sub(r"[,\.;\(\)\[\]«»]", " ", s)
        ...
        # 2) ищем «число + слово», где слово начинается с ч/м/с (или h/m/s)
        ...
        # 3) если ничего не найдено – пробуем просто числа (ччмс или ммс)
        if total == 0:
...
def _channel_is_target_from_title(title: str) -> bool:
    """Проверка, что заголовок окна Discord относится к нужному каналу."""
...
# ----------------------------------------------------------------------
# Варианты команд, которые может вернуть bump‑бот
COMMAND_PATTERNS = [
    r"/up", r"/bump", r"/like",
    r"!\s*up", r"!\s*bump", r"!\s*like"
]

# Компилируем один регекс для ускорения (с флагом ignore‑case)
_COMMAND_REGEX = re.compile("|".join(COMMAND_PATTERNS), re.IGNORECASE)


def extract_latest_bump_message(full_text: str) -> Optional[str]:
    """
    Находит в полном тексте последние строки, где встречается одна из COMMAND_PATTERNS
    **и** рядом есть хотя бы одна цифра (это гарантирует, что строка относится к
    bump‑сообщению). Возвращает блок из максимум 5 последних подходящих строк.
    """
    if not full_text:
        return None
    ...
    # выбираем строки, где есть команда + цифра
    candidate = [ln for ln in lines
                 if _COMMAND_REGEX.search(ln) and re.search(r"\d", ln)]

    if not candidate:
        log_debug("В тексте не найдено строк с командами + цифрами")
        return None

    # берём лишь последние 5 (чтобы не захватывать старый чат)
    return "\n".join(candidate[-5:])


def is_bump_message(text: str) -> bool:
    """Простая проверка – есть команда и цифра."""
    return bool(re.search(r"/(?:up|bump|like)\b.*\d", text, re.IGNORECASE))


def _extract_time_from_line(line: str) -> Optional[int]:
    """
    Из строки, в которой уже найдена команда, вытаскивает количество секунд.
    Поддерживает любые разделители (пробелы, двоеточия, тире, запятые и пр.).
    """
    # Находим место, где начинается команда (регистронезависимо)
    match = re.search(r"(?i)(/up|/bump|/like|!\s*up|!\s*bump|!\s*like)", line)
    if not match:
        return None

    after_cmd = line[match.end():]                # всё, что после команды
    after_cmd = after_cmd.strip(" :‑–—,.;|#")       # убираем «мусор»

    # Пробуем спарсить любой оставшийся кусок как длительность
...
def parse_time_from_message(message_text: str) -> Dict[str, Any]:
    """
    Принимает любой текст (полный буфер, ответ /remaining и т. д.) и
    возвращает словарь:
    ...
    if not block:
        ...
        log_debug("Текст сообщения для отладки (первые 500 символов):")
    ...
    for cmd in ("/up", "/bump", "/like"):
        # ищем строку, где присутствует конкретная команда
...
# ----------------------------------------------------------------------
# AUTO‑COPY BUMP MESSAGE (Ctrl+A)
# ----------------------------------------------------------------------
def find_and_copy_bump_message() -> Optional[str]:
    """
    Делает Ctrl+A → Ctrl+C в окне Discord, берёт весь буфер
    и пытается вычленить блок с bump‑сообщением. При неудаче делает
    несколько попыток (MESSAGE_SCAN_RETRIES).
    """
    log_status("ПОИСК BUMP‑СООБЩЕНИЯ (Ctrl+A)")
    ...
    # запомним текущий буфер, чтобы вернуть его потом
    ...
    try:
        for attempt in range(1, MESSAGE_SCAN_RETRIES + 1):
            ...
            # клик в центр экрана – гарантируем, что фокус в Discord
            w, h = pyautogui.size()
            ...
            # отладочная печать (только первые 300 символов)
            if copied:
                log_debug("Скопировано (первые 300 символов):")
...
def send_message(text: str, double_enter: bool = False) -> bool:
    ...
    Возвращает True, если команда была отправлена.
    """
...
def _schedule_parsed_commands(task: Dict[str, Any]) -> None:
    ...
    for cmd in task["commands_to_schedule"]:
        ...
        # небольшая «подушка» в 10 сек, чтобы гарантировать, что cooldown уже прошёл
        exec_time = now + secs + 10
        subtask = {
...
def execute_scheduled_tasks() -> None:
    ...
    with state_lock:
        ...
        # удаляем выполненные/ошибочные задачи из списка
        for t in completed:
            scheduled_tasks.remove(t)

def cleanup_old_tasks(max_age_seconds: int = 300) -> None:
    """
    Удаляем из расписания задачи, у которых время уже прошло более
    `max_age_seconds` назад (по умолчанию 5 минут).
    """
    now = time.time()
    with state_lock:
...
def execute_bump_tasks() -> None:
    ...
    now = time.time()
    for task in bump_tasks[:]:                     # копируем, чтобы можно было менять список
        status = task["status"]
        ...
        # ----------- отправка команды -----------
            ...
            if send_message(task["command"], task["double_enter"]):
                ...
                task["response_deadline"] = now + 5   # подождём 5 сек ответа
        ...
        # ----------- финал -----------
            ...
            # удаляем задачу из списка, чтобы не держать её в памяти
            bump_tasks.remove(task)
            # (при ошибке) просто продолжаем – пользователь увидит её в консоли
...
def main_loop() -> None:
    ...
    try:
        while True:
            # открываем меню по нажатию горячей клавиши
            ...
            # отложенные однократные команды
            execute_scheduled_tasks()

            # BUMP‑задачи (автопарсинг)
            execute_bump_tasks()

            # периодически чистим «протухшие» задачи
        ...
        # выводим полную трассировку – помогает в отладке
